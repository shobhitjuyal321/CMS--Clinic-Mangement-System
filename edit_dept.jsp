     ...
     <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                <!-- Start Page Content -->
            <form method="POST" id="form1" runnat="server">
                <center>
                    <h2><b>UPDATE DEPARTMENT</b></h2>
                    <table>     
                        
                    <tr><td><label id="dept_name"><b>Department Name: </b></label>
                    <input type="hidden" id="deptname" name="deptname" value="<%=request.getParameter("deptname")%>">
                    <input type="text" name="new_name" id="new_name" value="<%=request.getParameter("deptname")%>" size="37" /></td></tr>     
                  
                    <tr><td><label name="doc" id="doc" ><b>Doctor InCharge: </b></label>                     
                    <input type="text" name="doc" id="doc" value="<%String dcname=request.getParameter("docname");
                           if(dcname.equals("") || dcname.equals("null")){
                               out.println("No Doctor assigned");                               
                           }
                           else{
                               out.println(dcname);  
                           }
                    %>" size="37" readonly/></td></tr> <br>   
                    
                    <tr><td><label><b>Change Doctor InCharge: </b></label>        
                            
<%
    try{
        Class.forName("com.mysql.jdbc.Driver");
        Connection connection = (Connection) DriverManager.getConnection("jdbc:mysql://localhost:3306/project", "root", "");
        String name = request.getParameter("docname");
        Statement statement = connection.createStatement() ;

        resultset =statement.executeQuery("select name from doctor");
%>
                    <select name="new_doc" id="new_doc" style="width: 270px;">
            <option ><%= name%></option>
...
<%
            try {
                        ...
                        String newdoc = request.getParameter("new_doc");  
                        ...
                        Class.forName("com.mysql.jdbc.Driver");
                            ...
                            rs.next(); {                                 
                                ...
                                if (name.equals(dname)) {  
                                    session.setAttribute("already","Department "+name+" already Exists");
                                    response.sendRedirect("manage_dept.jsp");
                                }
                            }
                            ...
                            ps.setString(1, name);
                            ps.setString(2, newdoc);
                            ps.executeUpdate();
                            conn.close();
                            
                            session.setAttribute("adddept","Department "+name+" updated successfully");
                            response.sendRedirect("manage_dept.jsp");                            
                } 
