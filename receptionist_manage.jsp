...
<head>
    ...
    <title>Receptionist Manage</title>
...
%>                                
        ...
        <% 
                ...
                delete_status=request.getParameter("delete_status");
                    ...
                    {            
                        x=stmt.executeUpdate("Delete from patient where name='"+dname+"'");
                    }
                }
        %>
    ...
    %> 
        <tr>
        <th height="35" colspan="9"><div align="center"> Patient <%=dname%> deleted successfully !! </div></th>
    <%  }   %>
        </tr>
...
rs=stmt.executeQuery("select name, age, mobile, email, address, clinic from patient ORDER BY name");
...
<td><div align="center"><a href="update_patient.jsp?patname=<%=rs.getString("name")%>&patage=<%=rs.getString("age")%>&patmobile=<%=rs.getString("mobile")%>&patemail=<%=rs.getString("email")%>&pataddr=<%=rs.getString("address")%>&patclinic=<%=rs.getString("clinic")%>">Edit</a></div></td>
